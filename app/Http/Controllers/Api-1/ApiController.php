...
class ApiController extends Controller
{
    ...
    public function ads(Request $request){
        ...
        $results = Ads::select('ads.id','ads_name','ads.service_type','ads.ads_image','ads.ads_price','category.category_name','city_name',DB::raw("(CASE WHEN (favorite.ads_id > 0) THEN '1' ELSE '0' END)as favorite_status"))
    ...
    public function ads_detail(Request $request){
        $ads_id = $request->id;
        $results = Ads::select('ads.*','profile_image','customer.first_name','customer.last_name','city_name','city.id as city_id','customer.created_at as join_date')
        ->leftJoin('city', function($join) {
            $join->on('city.id', '=', 'ads.city_id');
          })
    ...
    public function my_favroite(Request $request){
        ...
        $limit = $request->limit ?  $request->limit : 10;
        $customer_id = $request->customer_id;  
        $results = Favorite::select('ads_name','ads.service_type','city_name','ads_image','ads_price')
       ...
       // echo "<pre>";print_r($request->get('')); die;
        ...
        if(count($arr_path)){
            for ($i=0; $i < count($arr_path); $i++) { 
                ...
                $name = "images-".time().rand(10000,200000).'.jpg';
                file_put_contents(public_path().'/images/listing/'.$name, base64_decode($arr_path[$i]));
            }
             $data['ads_image'] = $name;
        }
    ...
    public function user_ads(Request $request){
        ...
        $results = Ads::select('ads.id','ads_name','city_name','ads_image','ads.created_at','ads_status','ads_description',"service_type","ads_price")
        ->leftJoin('city', function($join) {
            $join->on('city.id', '=', 'ads.city_id');
          })
        ->orderBy('ads.id','desc')
    ...
    public function update_customer(Request $request){
     ...
     public function chat_list(Request $request){
        ...
        $limit = $request->limit ?  $request->limit : 10;
        $customer_id = $request->customer_id;
        $results = Chat::select('chat.*','ads.ads_name','ads.ads_image','s.first_name as sender_name','r.first_name as receiver_name')
         ->join('ads', function($join) {
    ...
    public function chat_history(Request $request){
       ...
       // echo "<pre>";print_r($results);die;
        $ads = Ads::select('id','customer_id','ads_name','ads_image','service_type','ads_price')->where("id",$ads_id)->first();
        if($ads){
            if($ads->ads_image){
                $ads->ads_image = url('public/images/listing/'.$ads->ads_image);
            }
            else{
